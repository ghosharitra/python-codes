...
proceeds around the circle in a fixed 
direction (clockwise). 
...
are removed), until only the last person remains, 
who is given freedom.

Given the total number of persons n and a number k 
which indicates that k-1 persons are skipped and 
...
Consider if n = 5 and k = 2, then the safe 
position is 3.
Firstly, the person at position 2 is killed, 
then person at position 4 is killed, 
then person at position 1 is killed. Finally, 
...
The first and only line of each test case consists 
of two space separated positive integers denoting n and k.
...
Explanation:
Testcase 1: here n = 2 and k = 1, then safe position 
is 2 as the person at 1st position will be killed.
